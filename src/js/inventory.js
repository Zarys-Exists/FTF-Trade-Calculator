...
function updateDisplayedItems() {
  ...
  if (allItems.length === 0) {
    itemList.innerHTML =
      '<div style="grid-column:1/-1;text-align:center;padding:2rem;color:#999;">Loading items…</div>';
    return;
  }
  ...
  if (filtered.length === 0) {
    itemList.innerHTML =
      '<p style="color:#999;text-align:center;padding:2rem;grid-column:1/-1;">No items found</p>';
    return;
  }
  ...
  const frag = document.createDocumentFragment();
  filteredItemCache
    .slice(0, ITEM_PAGE_SIZE)
    .forEach((i) => frag.appendChild(createModalItem(i)));
...
async function takeScreenshots() {
  ...
  await Promise.all(
    sorted.map(async (item) => {
      ...
      loaded++;
      if (loaded % 15 === 0)
        btn.textContent = `Loading images (${loaded} / ${sorted.length})…`;
    }),
  );
...
if (modal)
  modal.onclick = (e) => {
    if (e.target === modal) tryDismissModal();
  };
